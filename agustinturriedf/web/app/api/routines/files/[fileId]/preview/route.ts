...
type RoutineFilePreviewRouteContext = {
  params: Promise<{
    fileId: string;
  }>;
};

const resolveRoutineFileId = async (context: RoutineFilePreviewRouteContext) => {
...
const getPreviewMimeType = (type: "pdf" | "xls" | "xlsx") => {
  if (type === "pdf") return "application/pdf";
  if (type === "xls") return "application/vnd.ms-excel";
  return "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
};

export const GET = async (_request: Request, context: RoutineFilePreviewRouteContext) => {
  try {
    ...
    return new Response(fileBuffer, {
      status: 200,
      headers: {
        "Content-Type": getPreviewMimeType(routineFile.type),
        "Content-Disposition": `inline; filename="${encodeURIComponent(routineFile.name)}"`,
      },
    });
  } catch (error) {
